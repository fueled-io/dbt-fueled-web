...
with link_clicks as (
  select distinct
    ...
    first_value(ev.unstruct_event_com_fueledanalytics_fueled_link_click_1_0_1.target_url)
      over(partition by ev.page_view_id
      order by ev.derived_tstamp desc
      rows between unbounded preceding and unbounded following)
  ...
  where
    ...
    and ev.unstruct_event_com_fueledanalytics_fueled_link_click_1_0_1.target_url is not null -- only include link click events
)
